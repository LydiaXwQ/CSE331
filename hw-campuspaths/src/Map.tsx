...
class Map extends Component<MapProps, MapState> {
    ...
    render() {
        console.log(this.props.parsedEdges);
        const lines = this.props.parsedEdges.map((lineObj, index) => {
            const x1 = lineObj.start.x;
            const y1 = lineObj.start.y;
            const x2 = lineObj.end.x;
            const y2 = lineObj.end.y;
            return <MapLine key = {index} color={"red"} x1={x1} y1={y1} x2={x2} y2={y2}/>;
        })
